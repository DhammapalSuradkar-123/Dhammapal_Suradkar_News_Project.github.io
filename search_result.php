...
<div class="container">
  <div class="row">
    <div class="main1">
       ...
       ?>
      <h2 id="search"><span>Search_Query :</span> <?php echo $_GET['search']; ?></h2>
      <hr class="b-search">
      <?php
         include 'connection.php';
         if(isset($_GET['page'])){
         ...
         $sql = "SELECT post.post_id,post.tital,post.description,post.post_date,post.images,
                 category.category_name,user.Username,user.user_id,category.category_id
                 FROM post LEFT JOIN category ON post.category = category.category_id
                 LEFT JOIN user ON post.author = user.user_id
                 WHERE post.tital LIKE '%{$search_query}%' or post.description LIKE '%{$search_query}%' or user.Username LIKE '%{$search_query}%'
      ...
      <div id="pagination">
        <ul>
          <?php
            $sql1 = "SELECT post.tital,post.description,user.Username
            ...
            if($page > 1){
              echo '<li><a href="search_result.php?search='.$search_query.'&page='.($page-1).'">Prev</a></li>';
            }
            for($i = 1; $i <= $total_page; $i++){
              ...
              // echo '<li><a class='.$active.' href="search_result.php?search='.$search_query.'&page='.$i.'">'.$i.'</a></li>';
              echo '<li><a class='.$active.' href="search_result.php?search='.$search_query.'&page='.$i.'">'.$i.'</a></li>';
            }
            if($page < $total_page){
              echo '<li><a href="search_result.php?search='.$search_query.'&page='.($page+1).'">Next</a></li>';
            }
          ?>
        </ul>
      </div>
