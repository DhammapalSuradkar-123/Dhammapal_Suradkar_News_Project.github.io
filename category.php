    ...
    <div class="main1">
       <?php
         ...
         $sql = "SELECT post.post_id,post.tital,post.description,post.post_date,post.images,
                 ...
                 WHERE post.category LIKE '%{$category_id}%'
                 LIMIT {$offset}, {$limit}";
      ...
      <div id="pagination">
        <ul>
          <?php
            $sql1 = "SELECT post.post_id,post.tital,post.description,post.post_date,post.images,
                    category.category_name,user.Username,category.category_id
                    FROM post LEFT JOIN category ON post.category = category.category_id
                    LEFT JOIN user ON post.author = user.user_id
                    WHERE category.category_id LIKE '%{$category_id}%'
                     ORDER BY post.post_id DESC";
            $squery1 = mysqli_query($conn, $sql1) or die("Error in Query : select2");
            $total_record = mysqli_num_rows($squery1);
            $total_page = ceil($total_record/$limit);
            if($page > 1){
              echo '<li><a href="category.php?category='.$category_id.'&page='.($page-1).'">Prev</a></li>';
            }
            for($i = 1; $i <= $total_page; $i++){
              ...
              echo '<li><a class='.$active.' href="category.php?category='.$category_id.'&page='.$i.'">'.$i.'</a></li>';
            }
            if($page < $total_page){
              echo '<li><a href="category.php?category='.$category_id.'&page='.($page+1).'">Next</a></li>';
            }
          ?>
        </ul>
      </div>
